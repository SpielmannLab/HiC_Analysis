...
mkdir -p "${WORK}/apa_nextflow_launchdir"
rsync -r --update * "${WORK}/apa_nextflow_launchdir"
cd "${WORK}/apa_nextflow_launchdir"
chmod +x bin/*
rm slurm*.out
...
nextflow run apa_main.nf -params-file apa_params.yaml -resume
