#!/usr/bin/env python
# This file is unchanged from the original file at https://github.com/tariks/peakachu/tree/master/diffPeakachu
...
# output
outfil1 = sys.argv[3].split('.')[0] + '.' + sys.argv[1].replace('_merged.loops', '.unique.loops')
with open(outfil1, 'w') as out:
    for line in unique1:
        tmp = list(map(str, line))
        out.write('\t'.join(tmp)+'\n')

outfil2 = sys.argv[3].split('.')[0] + '.' + sys.argv[2].replace('_merged.loops', '.unique.loops')
